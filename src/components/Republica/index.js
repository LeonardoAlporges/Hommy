...
class Republica extends Component {
  ...
  fAnimalSim = async (checked) => {
    if (this.state.filtroAnimalSim)
      ...
      await this.setState({ filtroAnimalSim: true, filtroAnimalNao: false });
  }
  fAnimalNao = async (checked) => {
    if (this.state.filtroAnimalNao)
      await this.setState({ filtroAnimalNao: false });
    else
      await this.setState({ filtroAnimalNao: true, filtroAnimalSim: false });
  }

  fMasc = async (checked) => {
    if (this.state.filtroMasc)
      await this.setState({ filtroMasc: false });
    else
      await this.setState({ filtroMasc: true, filtroFem: false, filtroMista: false });
  }

  fFem = async (checked) => {
    if (this.state.filtroFem)
      await this.setState({ filtroFem: false });
    else
      await this.setState({ filtroFem: true, filtroMasc: false, filtroMista: false });
  }

  fMista = async (checked) => {
    if (this.state.filtroMista)
      await this.setState({ filtroMista: false });
    else
      await this.setState({ filtroMista: true, filtroMasc: false, filtroFem: false });
  }
  ...
  filtro = async () => {
   await this.setState({ listaRepublicas: this.state.fullData })
    ...
    if ((this.state.filtroMasc) === true) {
      listaRepublicas = _.filter(this.state.listaRepublicas, { "genero": "Masculina" });
    }
    if ((this.state.filtroFem) === true) {
      listaRepublicas = _.filter(this.state.listaRepublicas, { "genero": "Feminina" });
    }
    if ((this.state.filtroMista) === true) {
      listaRepublicas = _.filter(this.state.listaRepublicas, { "genero": "Mista" });
    }
          ...
          >
            <View style={{
              ...
              <ListItem style={{ alignItems: "center", marginBottom: 10 }}>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fAnimalSim} checked={this.state.filtroAnimalSim} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >Sim</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fAnimalNao} checked={this.state.filtroAnimalNao} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >Não</Text>
              </ListItem>
              <Text> Tipo de república</Text>
              <ListItem style={{ alignItems: "center", marginBottom: 10 }}>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMasc} checked={this.state.filtroMasc} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >Masculina</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fFem} checked={this.state.filtroFem} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >Feminina</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMista} checked={this.state.filtroMista} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >Mista</Text>
              </ListItem>
              <Text>Capacidade de moradores</Text>
              <ListItem style={{ alignItems: "center", marginBottom: 10 }}>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMoradores2} checked={this.state.filtroMoradores2} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >2</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMoradores3} checked={this.state.filtroMoradores3} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >3</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMoradores4} checked={this.state.filtroMoradores4} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >4</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMoradores5} checked={this.state.filtroMoradores5} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >5</Text>
                <CheckBox style={{ alignSelf: "stretch" }} onPress={this.fMoradores6} checked={this.state.filtroMoradores6} />
                <Text style={{ alignSelf: "stretch", paddingHorizontal: 15 }} >6+</Text>
              </ListItem>
